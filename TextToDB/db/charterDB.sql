...
CREATE TABLE IF NOT EXISTS "texts"(
    ...
    manuscript VARCHAR-- Should refer to an external record of a manuscript
);
...
CREATE TABLE IF NOT EXISTS "markup_conversations"(
    id SERIAL PRIMARY KEY
    -- Reference column for markups added later
);

CREATE TABLE IF NOT EXISTS "markup_comments"(
    id BIGSERIAL PRIMARY KEY,
    creator INTEGER REFERENCES users(id) NOT NULL,
    created TIMESTAMPTZ NOT NULL, -- Determines the sequence of comments
...
CREATE TABLE IF NOT EXISTS "markup_associations"(
    ...
    character_id INTEGER REFERENCES symbols(id)
);

ALTER TABLE "markup_conversations"
    ADD markup INTEGER REFERENCES markups(id);